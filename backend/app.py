...
def register():
    ...
    if email in USERS: return jsonify({'message':'User exists'}), 400
...
def login():
    ...
    email = data.get('email'); password = data.get('password')
    u = USERS.get(email)
    if not u or u.get('password')!=password: return jsonify({'message':'Invalid credentials'}), 400
